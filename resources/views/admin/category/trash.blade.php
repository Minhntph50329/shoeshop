...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Danh mục đã xóa</h2>
            <p class="text-sm text-slate-500">Khôi phục hoặc xóa vĩnh viễn các danh mục trong thùng rác</p>
        </div>
        <a href="{{ route('admin.category.index') }}" class="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm font-semibold transition">
            <i data-lucide="arrow-left" class="w-4 h-4"></i> Danh sách danh mục
        </a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                    @forelse($trashed as $cat)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="py-4 px-6 font-medium text-slate-500">#{{ $cat->id }}</td>
                            <td class="py-4 px-6 font-semibold text-slate-800">{{ $cat->name }}</td>
                            <td class="py-4 px-6 text-slate-500 text-xs">{{ $cat->deleted_at->format('d/m/Y H:i') }}</td>
                            <td class="py-4 px-6 text-right space-x-2">
                                <form action="{{ route('admin.category.restore', $cat->id) }}" method="POST" class="inline-block">
                                    @csrf
                                ...
                                <form action="{{ route('admin.category.forceDelete', $cat->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Bạn có chắc muốn xóa VĨNH VIỄN danh mục này?')">
                                    @csrf
                                    @method('DELETE')
