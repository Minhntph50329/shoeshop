...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Sản phẩm đã xóa</h2>
            <p class="text-sm text-slate-500">Khôi phục hoặc xóa vĩnh viễn các sản phẩm trong thùng rác</p>
        </div>
        <a href="{{ route('admin.products.index') }}" class="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm font-semibold transition">
            <i data-lucide="arrow-left" class="w-4 h-4"></i> Danh sách sản phẩm
        </a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                    @forelse($trashed as $product)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="py-4 px-6 font-medium text-slate-500">#{{ $product->id }}</td>
                            <td class="py-4 px-6 font-semibold text-slate-800">{{ $product->name }}</td>
                            <td class="py-4 px-6 font-bold text-slate-700">{{ number_format($product->price) }}đ</td>
                            <td class="py-4 px-6 text-slate-500 text-xs">{{ $product->deleted_at->format('d/m/Y H:i') }}</td>
                            <td class="py-4 px-6 text-right space-x-2">
                                ...
                                <form action="{{ route('admin.products.forceDelete', $product->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Bạn có chắc muốn xóa VĨNH VIỄN sản phẩm này?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-8 text-center text-slate-400">Thùng rác trống.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
