...
@section('title', 'Thùng rác - Quản lý Banner')
@section('page_title', 'Thùng rác - Banner đã xóa')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Thùng rác Banner</h2>
            <p class="text-sm text-slate-500">Khôi phục hoặc xóa vĩnh viễn các Banner trong thùng rác</p>
        </div>
        <a href="{{ route('admin.banner.index') }}" class="flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition">
            <i data-lucide="arrow-left" class="w-4 h-4"></i> Danh sách Banner
        </a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-xs text-slate-600">
                    @forelse($trashed as $banner)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="p-4 font-mono text-slate-400">#{{ $banner->id }}</td>
                            <td class="p-4">
                                <img src="{{ asset($banner->image) }}" alt="{{ $banner->title }}" class="w-20 h-12 object-cover rounded-lg border border-slate-200 bg-slate-50">
                            </td>
                            ...
                            <td class="p-4 text-right space-x-2">
                                <form action="{{ route('admin.banner.restore', $banner->id) }}" method="POST" class="inline-block">
                                    @csrf
                                ...
                                <form action="{{ route('admin.banner.forceDelete', $banner->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Bạn có CHẮC CHẮN muốn XÓA VĨNH VIỄN Banner này?')">
                                    @csrf
                                    @method('DELETE')
